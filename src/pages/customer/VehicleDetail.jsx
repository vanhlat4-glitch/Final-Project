...
export default function VehicleDetail() {
  ...
  if (!vehicle) {
    return (
      <DashboardLayout title="Không tìm thấy xe">
        <div className="card">
          <p className="mb-16">Không tìm thấy thông tin xe yêu cầu hoặc xe đã được gỡ khỏi hệ thống.</p>
  ...
  return (
    <DashboardLayout title={vehicle.name} subtitle={`${vehicle.brand} · ${vehicle.type} · ${vehicle.location}`}>
      ...
      <div className="detail-hero mt-16">
        <div>
          ...
          <div className="card mt-16">
            <h3 style={{ fontSize: 16, marginBottom: 12, fontWeight: 700 }}>Thông số kỹ thuật</h3>
            <div className="kpi-row" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Hãng xe</span>
                <strong style={{ fontSize: 15, color: "var(--ink)", marginTop: 4 }}>{vehicle.brand}</strong>
              </div>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Kiểu dáng</span>
                <strong style={{ fontSize: 15, color: "var(--ink)", marginTop: 4 }}>{vehicle.type}</strong>
              </div>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Số chỗ</span>
                <strong style={{ fontSize: 15, color: "var(--ink)", marginTop: 4 }}>{vehicle.seats} chỗ</strong>
              </div>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Hộp số</span>
                <strong style={{ fontSize: 15, color: "var(--ink)", marginTop: 4 }}>{vehicle.transmission}</strong>
              </div>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Nhiên liệu</span>
                <strong style={{ fontSize: 15, color: "var(--ink)", marginTop: 4 }}>{vehicle.fuel}</strong>
              </div>
              <div className="card" style={{ background: "var(--paper)", padding: "12px 14px", border: "1px solid var(--line)" }}>
                <span className="text-muted text-sm">Khu vực</span>
          ...
          <div className="card mt-16">
            <h3 style={{ fontSize: 16, marginBottom: 10, fontWeight: 700 }}>Mô tả phương tiện</h3>
            <p className="text-sm" style={{ lineHeight: 1.6, color: "var(--ink)" }}>{vehicle.description}</p>

            {vehicle.features && vehicle.features.length > 0 && (
              <div style={{ marginTop: 14 }}>
                <strong style={{ fontSize: 13, display: "block", marginBottom: 8 }}>Trang bị & Tiện nghi nổi bật:</strong>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {vehicle.features.map((feat, idx) => (
                    <span key={idx} className="plate" style={{ background: "#f3f4f6", color: "var(--ink)", borderColor: "var(--line)" }}>
                      ✓ {feat}
                    </span>
                  ))}
                </div>
          ...
          {vehicleReviews.length > 0 && (
            <div className="mt-24">
              <h3 style={{ fontSize: 16, marginBottom: 12, fontWeight: 700 }}>Đánh giá từ khách hàng ({vehicleReviews.length})</h3>
                ...
                {vehicleReviews.map((r) => (
                  <div className="card" key={r.id || r._id}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      ...
                      <span className="text-sm text-muted">{r.createdAt || "Gần đây"}</span>
                    </div>
                    ...
                    {r.providerReply && (
                      <div style={{ marginTop: 8, padding: "8px 12px", background: "var(--paper)", borderRadius: 6, fontSize: 12 }}>
                        <strong>Chủ xe phản hồi:</strong> {r.providerReply}
                      </div>
                    )}
                  </div>
                ))}
        ...
        <div>
          <div className="card" style={{ position: "sticky", top: 20 }}>
            <h3 style={{ fontSize: 18, marginBottom: 14, fontWeight: 700 }}>Bảng giá thuê</h3>

            <div style={{ background: "var(--paper)", padding: 14, borderRadius: "var(--radius-sm)", border: "1px solid var(--line)", marginBottom: 16 }}>
              <div className="flex-between mb-8">
                <span className="text-sm text-muted">Thuê theo giờ:</span>
                <span className="price" style={{ color: "var(--signal-dark)", fontSize: 18 }}>
                  {formatVND(pricePerHour)}<small>/giờ</small>
                </span>
              </div>
              <div className="flex-between">
                <span className="text-sm text-muted">Thuê trọn ngày:</span>
            ...
            <div style={{ borderTop: "1px solid var(--line)", paddingTop: 14, marginBottom: 16 }}>
              <h4 style={{ fontSize: 13.5, marginBottom: 8, fontWeight: 600 }}>Cam kết dịch vụ Morent:</h4>
              <ul style={{ paddingLeft: 18, fontSize: 12.5, color: "var(--muted)", lineHeight: 1.6 }}>
                <li>Bảo hiểm vật chất 2 chiều tiêu chuẩn</li>
                <li>Hỗ trợ giao nhận xe tận nơi hoặc sân bay</li>
                <li>Vệ sinh, khử khuẩn xe sạch sẽ trước khi bàn giao</li>
                <li>Hỗ trợ sự cố kỹ thuật cứu hộ 24/7</li>
              </ul>
            </div>
